<template>
  <div class="main" ref="main">
    ...
    <button class="edit">
      <a href="/admin/addUser" class="editLink">Add User</a>
    </button>
    <div v-for="(user, index) in store2.users" :key="index" class="userItem">
      ...
      <button v-if="user.role === 'member'" class="change" @click="deleteUser(user.id, index)">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="16"
          height="16"
...
userItems.value = document.getElementsByClassName('userItem')
...
function edit() {
  if (changingUser.value.name.trim() !== '' && changingUser.value.name.trim().length > 2) {
    store3.changeUserData(changingUser.value)
    done.value.style.display = 'block'
  } else {
    alert('Something went wrong!')
    changingUser.value.name = ''
  }
}

function deleteUser(userId, index) {
  store3.deleteUser(userId)
  userItems.value[index].style.display = 'none'
}
</script>
